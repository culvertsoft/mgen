...
public class BinaryReader extends BuiltInReader {
	...
	private MGenBase readMGenObject(final boolean readTypeTag,
			final CustomType constraint) throws IOException {
	...
	private Object readEnumArray(final int n, final ArrayType constraint,
			final boolean b) throws IOException {
		final Enum<?>[] array = constraint != null ? (Enum<?>[]) constraint
				.newInstance(n) : new Enum<?>[n];
		for (int i = 0; i < array.length; i++)
			array[i] = readEnum(false,
					constraint != null ? (EnumType) constraint.elementType()
							: null);
		return array;
	}
	...
	private Object readObjectArray(final int n, final byte elemTypeTag,
		...
		for (int i = 0; i < array.length; i++)
			array[i] = readObject(elemTypeTag,
	...
	private ArrayList<Object> readList(final boolean tag,
	...
	private HashMap<Object, Object> readMap(final boolean doReadTag,
		...
		if (nElements > 0) {
			...
			final List<Object> keys = readElements(true,
					constraint != null ? constraint.keyType() : null);
			final List<Object> values = readElements(true,
	...
	private ArrayList<Object> readElements(final boolean doReadTag,
	...
	protected MGenBase instantiate(final short[] ids,
			final CustomType constraint) {
	...
	private Enum<?> readEnum(final boolean readTag, final EnumType constraint)
			throws IOException {
