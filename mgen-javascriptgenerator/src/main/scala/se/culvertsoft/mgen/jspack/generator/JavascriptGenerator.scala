...
class JavascriptGenerator extends Generator {
  ...
  override def generate(modules: java.util.List[Module], generatorSettings: java.util.Map[String, String]): java.util.List[GeneratedSourceFile] = {
    ...
    //   MkInto(generatorSEttings) +
    modules.map(m=> MkModuleHashRegistry(m) + MkModuleClassRegistry(m))
...
//      val hashes = t.superTypeHierarchy().map(x => "\"" + x.typeId16BitBase64() + "\"").mkString(", ")
