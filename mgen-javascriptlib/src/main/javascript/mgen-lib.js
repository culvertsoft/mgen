(function() {
    ...
    var mGen = {
         ...
         * later used to instantiate classes. To create a ClassRegistry simply
         * call mGen.generate with a ClassRegistryBlueprint (generated by mgen-javascriptgenerator)
         *
         * For examples on how to use this please check the api tests located in "standard.spec.js".
         *
         ...
         * @param settings.warn              {bool} Set to true if warnings should be printed in the console, default is true
         ...
         * @param settings.validate          {bool} Set to true for type-validation of data on creation. Default true
         * @param settings.verify_transient  {bool} Set to true if transient fields should be removed on serialization. Default true
         * @param settings.remove_transient  {bool} Set to true if transient fields should be removed on object creation. Default false
        ...
        generate: function(ClassRegistryBlueprint, settings) {
            ...
            settings = {
                ...
                verify_transient: (typeof settings.verify_transient === "undefined") ? true : settings.verify_transient,
                remove_transient: (typeof settings.remove_transient === "undefined") ? false : settings.remove_transient
            };
            ...
            // Expose settings
            ClassRegistry.settings = settings;
            ...
            function createClass(obj) {
                var _c = {}; // the _c object is used to trick the function.name property to be correct for the class.
                // see https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function/name
                _c[tmpWrap(obj.__t)] = function(data, options) {
                    var field, default_construction = false;
                    ...
                    if (data) {
                        if (data.__t) {
                            //If the data does not fit the corresponding type then check if it's a derived type.
                            if (obj.__t != data.__t) {
                                if (isDerivedType(data.__t, obj.__t)) {
                                    return new(ClassRegistry.getClass(tmpWrap(data.__t)))(data);
                                } else {
                                    throw Error(" Tried to create " + tmpWrap(obj.__t) + "("+obj.__t+") but got " + tmpWrap(data.__t) + " ("+data.__t+")");
                                }
                            }
                        }
                        if (options.validate) {
                    ...
                    // set default fields for fields that have default values and no data.
                    for (field in obj){
                        if (!obj.hasOwnProperty(field)) continue;
                        if( obj[field]["default"] !== undefined ) {
                            data[field] = obj[field]["default"];
                        }
                    }
            ...
            function createField(field, key, data, options) {
                if (hasFlag(field.flags, "transient") && options.remove_transient){
                    return null;
                }
            ...
            function isDerivedType(derived, base) {
                return derived.substr(0, base.length) === base;
            }
        ...
        jsonHandler: function(ClassRegistry) {
            ...
            JsonHandler.prototype.stringToObject = function(data, settings) {
                if (typeof data !== "string") throw Error("Create from string needs a string argument");
                var object = JSON.parse(data);
                return this.jsonToObject(object, settings);
            };

            JsonHandler.prototype.jsonToObject = function(object, settings) {
                return new(this.ClassRegistry.getClassWithId(object.__t))(object, settings);
            };

            JsonHandler.prototype.objectToString = function(object, settings) {
                if(this.ClassRegistry.settings.verify_transient || settings.verify_transient){
                    object = this.jsonToObject(object, {remove_transient: true});
                }
