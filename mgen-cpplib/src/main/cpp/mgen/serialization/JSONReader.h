...
public:
	...
	JSONReader(Stream& inputStream, const Registry& classRegistry) :
			m_inputStream(inputStream), m_jsonStream(&m_inputStream), m_classRegistry(
					classRegistry) {
	}
...
private:
	...
	MGenBase * readMgenObject(const Node& node, const bool constrained,
			const long long expTypeId, MGenBase * object = 0) {

		if (node.IsNull())
			return 0;

		const ClassRegistryEntry * entry = readMGenBaseHeader(node);

		return serialutil::readObjFromEntry(*this, m_classRegistry, node,
				object, entry, expTypeId, constrained);
	...
	const ClassRegistryEntry * readMGenBaseHeader(const Node& node) {
		...
		if (v.IsArray()) {
			...
			return m_classRegistry.getByTypeIds16BitBase64(typeIdsBase64);
