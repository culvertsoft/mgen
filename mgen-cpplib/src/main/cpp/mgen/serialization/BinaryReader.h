...
private:
	...
	MGenBase * readMgenObject(const bool verifyTag, const bool constrained,
		...
		return serialutil::readObjFromEntry(*this, m_classRegistry,
				context_ignore, object, entry, expectTypeId, constrained);
	...
	void skipMap() {
		...
		if (sz > 0) {
			...
			throw StreamCorruptedException(
					"BinaryReader::skipMap: unexpected (<0) size");
		}
	}
	...
	void skip(const Type::TAG tag) {
		...
		default:
			throw UnexpectedTypeException(
					"BinaryReader::skipField(..): Unexpected tag");
		}
	}
#undef SKIP_CASE_READ
	...
	void read(std::string& v, const bool verifyTag) {
		...
		if (sz < 0) {
			throw StreamCorruptedException(
					"BinaryReader::read(string): String size < 0");
		} else if (sz == 0) {
			v = "";
		} else {
			v.resize(sz);
	...
	void verifyReadTagIf(const Type::TAG expTag, const bool check) {
		if (check) {
			...
			if (tag != expTag) {
				throw UnexpectedTypeException(
						S("BinaryReader::verifyReadTagIf: Unexpected tag ").append(
								S(expTag)).append(" but got ").append(S(tag)));
			}
		}
	}
