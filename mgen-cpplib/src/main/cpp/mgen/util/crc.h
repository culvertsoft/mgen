...
inline short calc16(const void * src, const int nBytes);
inline int calc32(const void * src, const int nBytes);

static unsigned int s_crc_32_table[] = { 0x00000000, 0x77073096, 0xee0e612c,
...
inline int calc32(const void * src, const int nBytes) {
    ...
    for (int i = 0; i < nBytes; i++)
        crc = crc >> 8 ^ (s_crc_32_table[(crc ^ buffer[i]) & 0xff]);
    crc = crc ^ 0xffffffff;
    return crc;
}
